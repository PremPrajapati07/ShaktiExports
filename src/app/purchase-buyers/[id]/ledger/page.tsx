...
export default async function PurchaseBuyerLedgerPage({ params }: { params: Promise<{ id: string }> }) {
  const { id: idStr } = await params
  const id = parseInt(idStr)
  if (isNaN(id)) notFound()

  const party = await prisma.purchaseBuyer.findUnique({ where: { id } })
  if (!party) notFound()
  ...
  return (
    <div className="animate-fade-in">
      <div style={{ marginBottom: '1.5rem' }}>
        <Link href="/purchase-buyers" className="text-muted hover:text-primary" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
      ...
      <header className="page-header" style={{ marginBottom: '2rem' }}>
        <div>
          ...
          <p className="page-subtitle">Detailed diamond stock history for this buyer branch</p>
        </div>
      </header>
              ...
              {ledger.length === 0 && (
                <tr>
                  <td colSpan={5} style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>
                    No stock transactions found for this buyer branch.
                  </td>
                </tr>
              )}
            </tbody>
